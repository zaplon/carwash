...
class posnet:
    ...
    def __init__(self, port="/dev/ttyACM0"):  # "/dev/ttyS0"
    ...
    def commit_transaction(self, wplacono, reszta, kasa="Kasa", kasjer="Kasjer", stanowisko= "Nr 1", metoda=0 ):
		command = "$y"
		if metoda == 1:
			metoda_string = u'Płatność kartą'
		args = [0, 0, 1, 0, 0, 0, 0, 1, 0, 1, 1]
		txtargs = "%s\r%s\r%s" % (kasa, kasjer, stanowisko)
		if metoda > 0:
			metoda_string = metoda_string.encode('utf8')
			args.append(metoda)
		txtargs += "\r%.2f/%.2f/0/%.2f/%.2f/" % (self._total, self._total, wplacono, reszta)
		self.command(command, args, txtargs)
